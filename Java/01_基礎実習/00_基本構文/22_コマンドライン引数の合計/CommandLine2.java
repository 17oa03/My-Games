...
public class CommandLine2 {
	...
	public static void main(String[] args) {
		...
		if (args.length == 2) { // コマンドライン引数の数が 2 の場合
			int seisu1 = Integer.parseInt(args[0]); // Integer クラスの parseInt メソッドが文字列を整数に変換
			int seisu2 = Integer.parseInt(args[1]); // Integer クラスの parseInt メソッドが文字列を整数に変換
			int kotae = seisu1 + seisu2; // 合計の計算
			System.out.println("答えは " + kotae + " です。"); // 出力
		} else { // それ以外の場合
			System.out.println("コマンドライン引数エラー"); // 出力
		}
	}
}
